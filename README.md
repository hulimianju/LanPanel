...
> 当前进度：**阶段 1（导航面板）、阶段 2（设备发现）、阶段 3（IP 自动跟随与通知）已完成**，Docker 镜像、飞牛 fpk、OpenWrt ipk 均可构建。
> 设计稿：`design/` 目录。
...
面板卡片可以按 MAC 绑定到设备。绑定的方式有三种：
- 在设备详情里点「加入面板」，自动完成绑定；
- 编辑卡片时，如果地址里的 IP 对应一台已发现的设备，一键绑定；
- 从下拉框里手动选择设备。

绑定之后：

- **地址自动更新**：每次扫描后，只要设备当前 IP 与卡片地址不一致，就改写卡片地址里的 IP。端口、路径、参数都保持不变；地址用的是域名或公网 IP 时不改。程序启动时也会先按设备库对账一次。
- **状态点**：卡片右上角显示设备状态：
  - 🟢 在线
  - 🟡 设备在线，但上次扫描时该端口没有响应（例如服务挂了）
  - ⚪ 离线（离线卡片会整体变淡）
- **离线防抖**：设备连续两次扫描未出现，才判为离线，避免偶发丢包造成误报。

### 通知

在「设置 → 通知」里配置推送渠道，支持以下几种：
- 企业微信群机器人
- 钉钉群机器人
- 飞书群机器人
- Bark
- Server 酱
- 通用 Webhook（POST JSON，可对接 Home Assistant、Node-RED）

可推送的动态有：新设备、IP 变化、已绑定设备的离线和恢复在线。一次扫描产生的多条动态会合并成一条消息。推送地址只保存在服务端，不会下发给访客。所有动态也会记录在「扫描任务」页的「设备动态」里。
